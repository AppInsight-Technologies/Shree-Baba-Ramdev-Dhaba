...
class _SingleproductScreenState extends State<SingleproductScreen>
  ...
  void initState() {
    ...
    initialRoute(() async {
      ...
      debugPrint("varid........" +
          widget._varid.toString() +
          "  " +
          widget._productID.toString());
      _preduct = ProductRepo().getRecentProduct(widget._productID.toString());
      debugPrint("_preduct..." + await _preduct.toString());
      ProductController()
          .getprodut(widget._varid.toString(), "")
          .whenComplete(() {
  ...
  addtoshoppinglisttwo(i) async {
    ...
    Provider.of<BrandItemsList>(context, listen: false)
        .AdditemtoShoppinglist(itemid.toString(), varid.toString(),
            shoplistDataTwo.itemsshoplist[i].listid!)
        .then((_) {
      Provider.of<BrandItemsList>(context, listen: false)
          .fetchShoppinglist()
          .then((_) {
  ...
  Widget handler(int i) {
    if (int.parse(singleitemvar[i].varstock!) <= 0) {
      return (varid == singleitemvar[i].varid)
          ...
          : Icon(Icons.radio_button_off_outlined, color: ColorCodes.grey);
    } else {
      return (varid == singleitemvar[i].varid)
          ? Icon(Icons.radio_button_checked_outlined,
              color: ColorCodes.mediumBlueColor)
          : Icon(Icons.radio_button_off_outlined, color: ColorCodes.blackColor);
    }
  }
  ...
  Future<String> createShareLink(String singleProduct, String productID) async {
    ...
    final DynamicLinkParameters dynamicLinkParameters = DynamicLinkParameters(
      ...
      iosParameters: IOSParameters(
          bundleId: IConstants.androidId, appStoreId: IConstants.appleId),
      socialMetaTagParameters: SocialMetaTagParameters(
          title: store.singelproduct != null
              ? store.singelproduct!.item_description
              : "",
          //  description: store.singelproduct!=null?store.singelproduct!.item_description:"",
          imageUrl: Uri.parse(
              /*"https://demo.grocbay.com/uploads/items/images/potato.webp"*/ store
                  .singelproduct!.itemFeaturedImage
                  .toString())),
    );

//Short link
    final ShortDynamicLink shortLink =
        await (await FirebaseDynamicLinks.instance)
...
//Long link
  ...
  AppBarMobile(
      Map<String, dynamic> routeArgs, SellingItemsList sellingitemData) {
    return AppBar(
      toolbarHeight: 55.0,
      ...
      leading: IconButton(
        ...
        onPressed: () {
          ...
          /*  if(route!=null){
            print("current route....."+route.settings.name!+"   "+Routename.SingleProduct.name);
          ...
          if (GoRouter.of(context).navigator!.canPop())
            Navigation(context, navigatore: NavigatoreTyp.Pop);
          //Navigation(context, navigatore: NavigatoreTyp.PopUntill,name: Routename.SellingItem);
          else
            Navigation(
              context,
              navigatore: NavigatoreTyp.homenav,
              ...
              onTap: () {
                ...
                if (_isIOS) {
                  Share.share(S.current.download_app +
                      IConstants.APP_NAME +
                      '${S.current.from_app_store} https://apps.apple.com/us/app/id' +
                      IConstants.appleId);
                } else {
                  /*  Share.share(S.current.download_app +
                      IConstants.APP_NAME +
          ...
          (PrefUtils.prefs!.containsKey("apikey"))
              ...
              color: Theme
                  ...
                  .primaryColor,
                        ...
                        Image.asset(Images.addToListImg,
                            width: 25, height: 25, color: ColorCodes.iconColor),
                  ),
                )
              : SizedBox.shrink(),
      ...
      flexibleSpace: Container(
        decoration: BoxDecoration(
            gradient: LinearGradient(
                ...
                colors: <Color>[Colors.transparent, ColorCodes.appbarColor2])),
      ),
    );
  }
