...
*/
  ...
  Future<void> checkLocation() async {
    ...
    addressdata = (VxState.store as GroceStore).userData;
    try {
      final response = await http.post(IConstants.isEnterprise && Features.ismultivendor?Api.checkLocationmultivendor:Api.checkLocation, body: {
...
*/
    print("deli charge text...confirm order"+minOrdAmount.toString() + "...."+_minimumOrderAmountNoraml.toString());
    for(int i=0;i<productBox.length;i++)
        ...
        if (productBox[i].varStock == "0" || productBox[i].status == "1") {
          ...
          if ((productBox[i].durationType == "" ||
            ...
            if ((productBox[i].eligibleForExpress == "0" ||
              ...
              for (int i = 0; i < DefaultSlot.length; i++) {
                ...
                DefaultTota1 + (double.parse(
                    ...
                    (double.parse(DefaultSlot[i].price!) *
                        int.parse(DefaultSlot[i].quantity!))
                        .toString()));
                debugPrint("minOrdAmount..."+minOrdAmount.toString()+"  "+DefaultTota1.toString());

              }
              if(toppingsdefault) {
                debugPrint("toppingsdefault....");
              ...
              debugPrint("toppingsdefault....total.."+DefaultTota1.toString());
          ...
          if (((productBox[i].durationType == "" ||
            ...
            if(toppingsSecondExpress) {
              for (int k = 0; k < ExpressDetails.length; k++) {
                debugPrint("express length..."+ExpressDetails[k].toppings_data!.length.toString());
                debugPrint("express type..."+ExpressDetails[k].type.toString());
      ...
      ListView.builder(
          ...
          itemBuilder: (_, i)
          {
            ...
            for(int j = 0; j < newMap1!.values.elementAt(i).length; j++) {
              ...
              DefaultTimeTotal+(double.parse(
                  (double.parse(newMap1!.values.elementAt(i)[j].quantity!) * double.parse(newMap1!.values.elementAt(i)[j].weight!)).toString()))
                  :DefaultTimeTotal+(double.parse(
              ...
              if (!_checkaddress) {
                ...
                if (_checkmembership) {
                  if (double.parse((CartCalculations.totalMember).toStringAsFixed(
                    ...
                    prefs!.setString("isPickup", "no");
                    debugPrint("final..."+deliveryamount.toString()+"  "+deliveryDateamount1.toString()+"  "+deliveryTimeamount1.toString());
                    finalSlotDelivery=  deliveryamount + deliveryDateamount1 + deliveryTimeamount1;
                    finalExpressDelivery= deliveryExpressamount + deliverySlotamount + deliveryfinalexpressdate + deliveryfinalexpressTime;
                    debugPrint("final..."+finalSlotDelivery.toString()+"   "+finalExpressDelivery.toString());
                ...
                else
                {
                  if (double.parse((CartCalculations.total).toStringAsFixed(
                    ...
                    prefs!.setString("isPickup", "no");
                    debugPrint("final...1.."+deliveryamount.toString()+"  "+deliveryDateamount1.toString()+"  "+deliveryTimeamount1.toString());
                    finalSlotDelivery=  deliveryamount + deliveryDateamount1 + deliveryTimeamount1;
                    finalExpressDelivery= deliveryExpressamount + deliverySlotamount + deliveryfinalexpressdate + deliveryfinalexpressTime;
                    debugPrint("final..."+finalSlotDelivery.toString()+"   "+finalExpressDelivery.toString());
...
*/
    ...
    return  AppBar(
      ...
      leading: IconButton(
          ...
          onPressed: () async{
            ...
            print("if....else gradient prev"+widget.prev["prev"].toString());
            if(widget.prev["prev"].toString() == "cart_screen") {
              print("if....gradient");
              PrefUtils.prefs!.setString("confirmback", "yes");
      ...
      title: Text(
        ...
        style: TextStyle(color: IConstants.isEnterprise && !Features.ismultivendor && !Features.isWebTrail?ColorCodes.menuColor:ColorCodes.iconColor,fontWeight: FontWeight.normal),
      ),
  ...
  _updateCart(int qty,CartStatus cart,String varid,String price,String increment) {
    switch (cart) {
      ...
      case CartStatus.remove:
        cartcontroller.update((done){
          ...
          //   _isAddToCart = !done;
          // });
        },price: price.toString(), quantity: "0",weight: "0", var_id: varid,increament: increment, cart_id: "",toppings: "",
          topping_id: "",);
      ...
      case CartStatus.decrement:
        cartcontroller.update((done){
          ...
          quantity: (qty - 1).toString(),
          weight: (qty-1).toString(),
          var_id: varid,increament: increment, cart_id: "",toppings: "",
          topping_id: "",);
        // TODO: Handle this case.
        break;
    }
