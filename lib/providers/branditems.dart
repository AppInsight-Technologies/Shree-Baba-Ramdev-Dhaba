...
class BrandItemsList with ChangeNotifier {
     ...
     try {
      ...
      if (responseJson.toString() == "[]") {
        ...
        for (int i = 0; i < data.length; i++) {
          debugPrint("push notification data...."+data[i]['pushNotificationModule'].toString());
          ...
          if(!Features.ismultivendor)IConstants.APP_NAME = data[i]['restaurant_name'].toString();
  ...
  Future<void> fetchPaymentMode() async {
    // imp feature in adding async is the it automatically wrap into Future.
   // debugPrint("ehfnf" + PrefUtils.prefs!.getString('branch').toString() + (VxState.store as GroceStore).CartItemList[0].branch!);
    try {
      _paymentitems.clear();
  ...
  Future<void> deliveryCharges(String addressid) async {
    print("address id....url"+addressid.toString());
    // imp feature in adding async is the it automatically wrap into Future.
   ...
   try {
      ...
      final responseJson = json.decode(utf8.decode(response.bodyBytes));
      print("delivery charges response...."+responseJson.toString());
      if (responseJson.toString() != "[]") {
        List data = [];
