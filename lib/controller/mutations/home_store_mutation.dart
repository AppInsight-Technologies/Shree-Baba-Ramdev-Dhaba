...
class HomeStoreScreenController  extends VxMutation<GroceStore>{
  ...
  Future<bool> perform() async{
    ...
    store!.homestore = await homeStorerepo.getData(ParamBodyData(user: user,lat: lat,long: long,refid: IConstants.refIdForMultiVendor));
    ...
    if(Features.ismultivendor) {
      print("contains branch...."+PrefUtils.prefs!.containsKey("branch").toString());
    //  if(!PrefUtils.prefs!.containsKey("branch")) {
        if ((VxState.store as GroceStore).CartItemList.isEmpty)
          PrefUtils.prefs!.setString(
              "branch", store!.homestore.data!.nearestStores![0].id.toString());
        print(
            "branch.......controller" + PrefUtils.prefs!.getString("branch").toString() + " " + store!.homestore.data!.pushNotificationModule!.toString() + "hello");
     }
    //}
