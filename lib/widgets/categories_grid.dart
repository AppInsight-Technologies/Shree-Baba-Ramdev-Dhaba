...
class _CategoriesGridState extends State<CategoriesGrid> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async {
      ...
      print("alll categprty id....alll categprty id...." +
          widget.allCategoryDetail.id.toString());
  ...
  Widget _category() {
    ...
    int widgetsInRow = 3;
    double aspectRatio = (deviceWidth - (20 + ((widgetsInRow - 1) * 10))) /
        widgetsInRow /
        180; //135;
    ...
    if (deviceWidth > 1200) {
      widgetsInRow = 9;
      aspectRatio = (Vx.isWeb && !ResponsiveLayout.isSmallScreen(context))
    ...
    return VxBuilder(
        ...
        builder: (ctx, store, VxStatus? state) {
          if (widget.allCategoryDetail.subCategory != null &&
              widget.allCategoryDetail.subCategory.length > 0)
            subcatData = widget.allCategoryDetail.subCategory
                .where((element) =>
          ...
          return subcatData.length > 0
              ...
              const EdgeInsets.fromLTRB(10.0, 10.0, 10.0, 5.0),*/
                                  itemCount: /*subNestedcategoryData.itemsubNested.length*/ subcatData
                                      .length,
                                  itemBuilder: (ctx, i) {
                                    debugPrint("items...." +
                                        subcatData.length.toString());
                                    print(subcatData[i].description!);
                                    return Card(
                                      color: /*Color(0xFFD0F0DE)*/ ColorCodes
                                          .backgroundcolor,
                                      shape: RoundedRectangleBorder(
                                        borderRadius: BorderRadius.circular(4),
                                      ),
