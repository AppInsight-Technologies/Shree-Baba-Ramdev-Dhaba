...
enum Routename{
  ...
  BannerProduct,NotSubcategory,Pages,SubcategoryScreen,Splash,PromocodeScreen,StoreHome,StoreScreen,SearchStore,RateReviewScreen,CustomerEngagementScreen,NotAvailability,SurveyProductScreen
...
class PageControler extends Navigations  {
  ...
  return HomeScreen(key: state.pageKey);},
      ...
      //GoRoute(path: "share", builder: (context, state)  =>  SingleproductScreen(state.queryParams["id"]!,state.queryParams["productId"]!),),
...
abstract class Navigations {
  initialRoute(Function sonload) async {
    ...
    if (Features.ismultivendor) {
      print("branch.......routeargs"+PrefUtils.prefs!.getString("branch").toString());
      print("multivendor rought ......"+ PrefUtils.prefs!.getString("area").toString()+"..."+PrefUtils.prefs!.getString("latitude").toString()+"...."+PrefUtils.prefs!.getString("longitude").toString());
      if ((VxState.store as GroceStore).homestore
          .toJson()
          .isEmpty) {
        print("multivendor rought ......1");
        if (Vx.isWeb) {
        ...
        print("initial Loading data multi");
        if (Features.ismultivendor)
          print("initial Loading data multi 1");
          auth.getuserProfile(onerror: () {
            print("multivendor rought ......2");
            print("Guest User");
  ...
  Navigation(BuildContext context,{Routename? name,required NavigatoreTyp navigatore,Map<String, String>? parms, Map<String, dynamic>? qparms}){
    ...
    switch(navigatore){
      case NavigatoreTyp.Push:
        print("params....push...."+parms.toString()+"name..."+name.toString());
